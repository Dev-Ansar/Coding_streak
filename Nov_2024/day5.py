...
Recursion Overview:
We started with the concept of recursion and tackled a problem to calculate the factorial of a number using both recursive and iterative approaches.
...
Explanation: The range(0, 1, -1) produces an empty sequence, so the loop doesn’t execute, and the function correctly returns 1 because the initial value of fact is 1.
...
Clarification: Type hints make your code more readable and easier to maintain by indicating the expected parameter types and return type. They also help with static analysis but are not enforced at runtime in Python.

Iterative vs. Recursive Approaches:
You asked which approach is more resource-intensive.
Clarification: The recursive approach is typically more resource-intensive due to higher memory usage (stack space) and function call overhead, while the iterative approach uses constant space and is generally more efficient.
'''